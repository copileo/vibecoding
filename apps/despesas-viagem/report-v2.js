const TRIP_APP_VERSION='1.1.1';
function reportFmt(n,mode){if(mode==='EUR')return eur(n);if(mode==='BRL')return brl(n*(data.rate||0));return `${eur(n)} · ${brl(n*(data.rate||0))}`}
function reportWrap(ctx,text,maxWidth,maxLines=99){const words=String(text||'').trim().split(/\s+/).filter(Boolean),lines=[];let line='',truncated=false;for(let i=0;i<words.length;i++){const word=words[i],test=line?`${line} ${word}`:word;if(ctx.measureText(test).width<=maxWidth){line=test;continue}if(line){lines.push(line);line='';if(lines.length>=maxLines){truncated=true;break;i--}}else{let chunk='';for(const ch of word){const next=chunk+ch;if(ctx.measureText(next).width>maxWidth&&chunk){lines.push(chunk);chunk=ch;if(lines.length>=maxLines){truncated=true;break}}else chunk=next}if(truncated)break;line=chunk}if(i<words.length-1&&lines.length>=maxLines){truncated=true;break}}if(line&&lines.length<maxLines)lines.push(line);else if(line&&lines.length>=maxLines)truncated=true;if(truncated&&lines.length){let last=lines[Math.min(maxLines,lines.length)-1];while(ctx.measureText(last+'…').width>maxWidth&&last.length>1)last=last.slice(0,-1);lines[Math.min(maxLines,lines.length)-1]=last+'…'}return lines.length?lines.slice(0,maxLines):['']}
