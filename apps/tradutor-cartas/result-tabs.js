...
CopileoAI.prototype.chatWithImage=async function(request){
  ...
  if(callNumber===1){
    ...
    resetTimer=setTimeout(()=>{callNumber=0},1500);
...
function renderSegments(segments,fallback){
  if(Array.isArray(segments)&&segments.length)return`<p>${segments.map(x=>{let t=esc(x.text||'');if(x.bold)t=`<strong>${t}</strong>`;if(x.italic)t=`<em>${t}</em>`;return t}).join('')}</p>`;
...
function ensureTabs(){
  ...
  if(!tabs){
    ...
    tabs.id='result-side-tabs';
    tabs.className='result-side-tabs';
    tabs.setAttribute('role','tablist');
    translation.before(tabs);
    tabs.addEventListener('click',event=>{const button=event.target.closest('[data-result-side]');if(!button)return;activeSide=button.dataset.resultSide;renderResult()});
  }
  return tabs;
}
function renderResult(){
  ...
  tabs.innerHTML=`<button class="result-side-tab ${activeSide==='front'?'active':''}" data-result-side="front" role="tab" aria-selected="${activeSide==='front'}">Frente</button>${hasBack?`<button class="result-side-tab ${activeSide==='back'?'active':''}" data-result-side="back" role="tab" aria-selected="${activeSide==='back'}">Verso</button>`:''}`;
  const card=activeSide==='back'&&hasBack?backResult:frontResult;
  if(!card)return;
  $('translation').innerHTML=renderCard(card);
  $('original').innerHTML=renderOriginal(card);
  const title=frontResult?.title?.translated||frontResult?.cardId||'Tradução';
  $('result-title').textContent=title;
  $('result-meta').textContent=[hasBack?'Frente e verso':'Frente',frontResult?.cardId,card?.side&&`Lado ${card.side}`].filter(Boolean).join(' · ');
...
async function loadLatestSaved(){
  ...
  try{
    ...
    const items=await new Promise((resolve,reject)=>{const request=tx.objectStore(HISTORY_STORE).getAll();request.onsuccess=()=>resolve(request.result);request.onerror=()=>reject(request.error)});
    db.close();
...
const result=$('result');
if(result){
  new MutationObserver(()=>{if(!result.hidden){if(frontResult)renderResult();else loadLatestSaved()}}).observe(result,{attributes:true,attributeFilter:['hidden']});
}

window.addEventListener('click',event=>{
  const recent=event.target.closest('.recent-item');
  if(recent&&!frontResult){setTimeout(loadLatestSaved,50)}
});
