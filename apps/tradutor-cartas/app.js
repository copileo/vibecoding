...
async function translateBoth(){
  ...
  try{
    ...
    phraseMatches=findPhraseMatches(currentResult,configuredPhrases);
...
function renderPhraseListStatus(){
  const element=$('phrase-list-status');
  if(!element)return;
  if(phraseListError){
    element.textContent='⚠ A lista de frases não pôde ser carregada. As correspondências não estão disponíveis.';
...
async function loadPhrases(){
  ...
  try{
    ...
    phraseListError=error;
    console.warn('Could not load shared phrase list',error);
    return[];
  }
}
...
function messageFor(e){if(e instanceof CopileoAIError)return e.message||'Falha na AI API.';return e?.message||String(e)}
