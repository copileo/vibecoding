...
export class CopileoAI {
  ...
  async chat(input, options = {}) {
    ...
    for (const key of ['temperature', 'max_output_tokens', 'metadata']) if (options[key] !== undefined) payload[key] = options[key];
  ...
  async #request(path, { method = 'GET', body, authenticated = true, retryAuth = true } = {}) {
    ...
    try {
      ...
      if (authenticated) { const token = await this.credentialsProvider.getToken(); if (token) headers.Authorization = `Bearer ${token}`; }
      ...
      if (!response.ok) {
        ...
        throw new CopileoAIError(providerError?.message || `Gateway request failed with HTTP ${response.status}.`, { code: providerError?.code || 'GATEWAY_ERROR', status: response.status, details: providerError?.details || payload });
      }
      return payload;
    } catch (error) {
      if (error instanceof CopileoAIError) throw error;
      if (error?.name === 'AbortError') throw new CopileoAIError('Gateway request timed out.', { code: 'TIMEOUT', cause: error });
...
export async function createImagePart(image, detail = 'auto') {
  ...
  if (typeof image === 'string') {
    ...
    if (/^data:image\/(jpeg|png|webp);base64,/i.test(image)) return { type: 'input_image', image_url: image, detail };
