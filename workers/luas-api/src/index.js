...
const WORKER_VERSION='1.7.2-debug.1';
...
export default {async fetch(request,env,ctx){
 ...
 if(url.pathname==='/health')return json({ok:true,service:'vibecode-luas-api',provider:'nta-gtfs-realtime',apiVersion:1,workerVersion:WORKER_VERSION,upstream:NTA_URL});
...
async function getFeed(key,ctx){
 ...
 const cacheKey=new Request('https://cache.vibecode.invalid/nta-trip-updates-v2-debug1');
...
function buildForecast(code,[name,...stopIds],feed){
 ...
 const now=Math.floor(Date.now()/1000);const departures=[];let tripUpdates=0;let stopUpdates=0;const sampleStopIds=[];
 for(const entity of feed.entity){
  const update=entity.tripUpdate||entity.trip_update||entity.TripUpdate;if(!update)continue;tripUpdates++;
  const trip=update.trip||update.Trip||{};const routeId=String(trip.routeId||trip.route_id||trip.RouteId||'');
  const items=update.stopTimeUpdate||update.stop_time_update||update.StopTimeUpdate||[];
  const destination=String(trip.tripHeadsign||trip.trip_headsign||trip.TripHeadsign||findDestination(items)||'Luas');
  for(const item of items){
   stopUpdates++;
   const rawStopId=item.stopId??item.stop_id??item.StopId??'';
   if(sampleStopIds.length<20&&rawStopId&&!sampleStopIds.includes(String(rawStopId)))sampleStopIds.push(String(rawStopId));
   if(!matchesAnyStop(rawStopId,expectedIds))continue;
   const event=item.departure||item.Departure||item.arrival||item.Arrival||{};const timestamp=toSeconds(event.time??event.Time);
 ...
 return {apiVersion:1,workerVersion:WORKER_VERSION,provider:'nta-gtfs-realtime',stop:{code,name,ids:stopIds,codes:STOP_CODES[code]||[]},updated:new Date().toISOString(),message:'Official NTA GTFS-Realtime forecast',departures:dedupe(departures).slice(0,12),diagnostics:{entities:feed.entity.length,tripUpdates,stopUpdates,matches:departures.length,sampleStopIds}};
}

function buildFeedDiagnostics(feed){
 const first=feed.entity[0]||{};
 const update=first.tripUpdate||first.trip_update||first.TripUpdate||{};
 const trip=update.trip||update.Trip||{};
 const stopUpdates=update.stopTimeUpdate||update.stop_time_update||update.StopTimeUpdate||[];
 const routeCounts={};const agencyCounts={};const tripSamples=[];const luasCandidates=[];
 for(const entity of feed.entity){
  const tu=entity.tripUpdate||entity.trip_update||entity.TripUpdate;if(!tu)continue;
  const descriptor=tu.trip||tu.Trip||{};
  const route=String(descriptor.routeId||descriptor.route_id||descriptor.RouteId||'(missing)');
  const tripId=String(descriptor.tripId||descriptor.trip_id||descriptor.TripId||'');
  routeCounts[route]=(routeCounts[route]||0)+1;
  const agency=String(descriptor.agencyId||descriptor.agency_id||descriptor.AgencyId||'(missing)');
  agencyCounts[agency]=(agencyCounts[agency]||0)+1;
  if(tripSamples.length<12)tripSamples.push({routeId:route,tripId,directionId:descriptor.directionId??descriptor.direction_id??descriptor.DirectionId??null,scheduleRelationship:descriptor.scheduleRelationship??descriptor.schedule_relationship??descriptor.ScheduleRelationship??null,stopIds:(tu.stopTimeUpdate||tu.stop_time_update||tu.StopTimeUpdate||[]).slice(0,4).map(item=>String(item.stopId??item.stop_id??item.StopId??''))});
  const searchable=`${route} ${tripId} ${JSON.stringify(descriptor)}`;
  if(luasCandidates.length<20&&/(LUAS|GREEN|RED|TRAM|LIGHT.?RAIL|TII)/i.test(searchable))luasCandidates.push({routeId:route,tripId,trip:sanitise(descriptor,3)});
 }
 const collections={};
 for(const [key,value] of Object.entries(feed)){
  if(key==='entity')continue;
  collections[key]=Array.isArray(value)?{type:'array',length:value.length,sample:sanitise(value.slice(0,2),3)}:{type:typeof value,sample:sanitise(value,3)};
 }
 return {workerVersion:WORKER_VERSION,upstream:NTA_URL,generatedAt:new Date().toISOString(),topLevelKeys:Object.keys(feed),entityCount:feed.entity.length,firstEntityKeys:Object.keys(first),firstEntity:sanitise(first,4),firstTripUpdateKeys:Object.keys(update),firstTripDescriptor:sanitise(trip,3),firstStopUpdate:sanitise(stopUpdates[0]||null,3),collections,topRoutes:topEntries(routeCounts,30),topAgencies:topEntries(agencyCounts,20),tripSamples,luasCandidates};
}

function sanitise(value,depth){
 if(depth<0)return '[depth-limit]';
 if(value===null||value===undefined||typeof value==='string'||typeof value==='number'||typeof value==='boolean')return value;
 if(Array.isArray(value))return value.slice(0,8).map(item=>sanitise(item,depth-1));
 if(typeof value==='object')return Object.fromEntries(Object.entries(value).slice(0,30).map(([key,item])=>[key,sanitise(item,depth-1)]));
 return String(value);
}
