...
const KNOWN_CODE_OVERRIDES={gln:'gle'};
...
export async function auditOfficialStops(ctx){
 ...
 const rows=APP_STOPS.map(([appCode,name])=>{
  const officialCode=resolveOfficialCode(appCode,catalog);
  const official=catalog.find(stop=>stop.code===officialCode);
  return {name,appCode,officialCode,status:official?'matched':'missing',codeDiffers:appCode!==officialCode,officialName:official?.name||null};
 });
...
function parseStopCatalog(xml){
 ...
 while((match=pattern.exec(xml))){
  const attrs=match[1];
  const code=(attribute(attrs,'abrev')||attribute(attrs,'abbr')||attribute(attrs,'code')).toLowerCase();
  const name=attribute(attrs,'pronunciation')||attribute(attrs,'name')||attribute(attrs,'stop');
  if(code&&name)stops.push({code,name,line:attribute(attrs,'line')||null});
 }
...
function resolveOfficialCode(appCode,catalog){
 ...
 const appName=APP_STOPS.find(([code])=>code===appCode)?.[1]||'';
 const wanted=normaliseName(appName);
 const exact=catalog.find(stop=>normaliseName(stop.name)===wanted);
 if(exact)return exact.code;
 const compatible=catalog.find(stop=>normaliseName(stop.name).includes(wanted)||wanted.includes(normaliseName(stop.name)));
...
async function fetchXml(url){
 ...
 try{const response=await fetch(url,{headers:{Accept:'application/xml,text/xml,*/*'},signal:controller.signal});if(!response.ok)throw new Error(`Official Luas API returned HTTP ${response.status}.`);return await response.text();}
 catch(error){if(error?.name==='AbortError')throw new Error('Official Luas API request timed out.');throw error;}
...
function parseOfficialXml(xml,code,officialCode){
 ...
 const stopName=attribute(root,'stop')||code.toUpperCase();const createdRaw=attribute(root,'created');const created=parseDublinLocal(createdRaw);const updated=created?created.toISOString():new Date().toISOString();const baseTime=created?.getTime()||Date.now();
 ...
 return {apiVersion:1,workerVersion:'1.8.2',provider:'luas-official-avls',stop:{code,name:stopName,officialCode},updated,message,departures,diagnostics:{source:'official-luas-pid',matches:departures.length,createdRaw,officialStopCode:officialCode}};
}
