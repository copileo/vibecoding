...
export function getConfig(env = {}) {
  ...
  return {
    ...
    rateWindow: number(env.RATE_LIMIT_WINDOW_SECONDS, 60),
    cacheTtl: number(env.CACHE_TTL_SECONDS, 3600),
  };
}
...
function parseModels(value) {
  ...
  return String(value).split(',').map(item => normalise(item.trim())).filter(Boolean);
}
...
function normalise(value) {
  ...
  if (typeof value === 'string') {
    return {
      id: value,
      provider: 'openai',
      capabilities: value.includes('embedding') ? ['embeddings'] : ['chat', 'responses'],
    };
  }
  if (typeof value === 'object' && value.id) {
    return {
      ...
      capabilities: Array.isArray(value.capabilities) ? value.capabilities : ['chat', 'responses'],
    };
  }
  return null;
}
